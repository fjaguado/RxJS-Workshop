...
import { map, Observable, Subscription, timer, zip } from 'rxjs';
import { ajax } from 'rxjs/ajax';
import { CoinbaseAPI, CoinGekoAPI } from '../../../../models/api.model';
import { ZIP_SECTION } from '../combination.data';

type ZipType = CoinbaseAPI | CoinGekoAPI[] | string;
...
export class ZipComponent {
  public ZIP_SECTION = ZIP_SECTION;
  ...
  public zipValues(): void {
    ...
    const zipArray = [coinbaseObserver, coingeckoObserver, thirdObserver];

    this.subscription = zip(zipArray).subscribe((val: ZipType[]) => {
      console.log(val);
      val.forEach((el) => {
        if (typeof el === 'string') {
...
const getStringTsCode = (): string => `
  ...
  export class ZipComponent {
    ...
    public zipValues(): void {
      ...
      this.subscription = zip(zipArray).subscribe(
        (val: ZipType[]) => val.forEach(el => {
            if (typeof el === 'string') {
              this.combinedValue += 'String value: ' + el';
              return;
            }
...
const getStringHTMLCode = (): string => `
  <div class="d-flex pt-3 column gap-3">
    ...
    <div class="col-6">
      <div class="form-text">zip() strings Subscription</div>
